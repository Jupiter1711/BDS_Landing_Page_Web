...
const Home = () => {
  ...
  const fetchProperties = async (params = {}) => {
    try {
      ...
      // Handle both search response format and normal response format
  ...
  const handleSearch = (filters) => {
    ...
    console.log('Search filters:', cleanFilters); // Debug
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-semibold text-blue-800">
                Kết quả tìm kiếm ({properties.length} chỗ ở)
              </h3>
              <p className="text-blue-600 text-sm mt-1">
                {Object.keys(searchParams).length > 0 && 
                  'Đang hiển thị kết quả theo bộ lọc của bạn'
                }
              </p>
            </div>
            <button
      ...
      {(searchLoading) ? (
        <div className="flex justify-center items-center h-64">
          <div className="text-gray-500">Đang tìm kiếm chỗ ở...</div>
        </div>
      ) : (
        <>
          ...
          {/* No Results Message */}
          {properties.length === 0 && Object.keys(searchParams).length > 0 && (
            <div className="text-center py-12">
              <div className="text-gray-500 text-lg mb-4">
                Không tìm thấy chỗ ở phù hợp với tiêu chí tìm kiếm của bạn.
              </div>
              <button
                onClick={handleResetSearch}
